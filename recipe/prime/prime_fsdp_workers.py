...
class PRIMERewardModelWorker(Worker):
    ...
    def _build_reward_ref_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            setattr(reward_model_config, "classifier_dropout", 0.0)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            dpo_acc = compute_dpo_accuracy(
                rm_scores, acc, eos_mask=eos_mask, n_samples=data.meta_info["n"]
            )
            dpo_acc_abs = compute_dpo_abs_accuracy(
                rm_scores, acc, eos_mask, n_samples=data.meta_info["n"]
            )
    ...
    def update_rm(self, data: DataProto):
        ...
        if self._is_offload_optimizer:
            load_fsdp_optimizer(
                optimizer=self.reward_optimizer, device_id=torch.cuda.current_device()
            )
