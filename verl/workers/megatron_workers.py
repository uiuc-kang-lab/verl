...
class CriticWorker(MegatronWorker):
    ...
    def _build_critic_model_optimizer(
        ...
        override_config_kwargs.update(override_model_config)
        self.share_embeddings_and_output_weights = getattr(
            critic_model_config, "tie_word_embeddings", False
        )
        update_model_config(
        ...
        if self.rank == 0:
            print(
                f"Model config after override: critic_model_config {critic_model_config}"
            )
    ...
    def load_checkpoint(self, checkpoint_path, del_local_after_load=True):
        self.hf_config = self.checkpoint_mananager.load_checkpoint(
            local_path=checkpoint_path, del_local_after_load=del_local_after_load
        )
